...
void zend_do_set(const znode *key, const znode *value TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);

	//opline->result_type = IS_TMP_VAR;
	//opline->result.var = get_temporary_variable(CG(active_op_array));
	opline->opcode = ZEND_SET;
	SET_NODE(opline->op1, key);
...
void zend_do_whether_cond(const znode *cond, znode *closing_bracket_token TSRMLS_DC) /* {{{ */
{
	int if_cond_op_number = get_next_op_number(CG(active_op_array));
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
...
void zend_do_in_op(znode *result, const znode *op1, const znode *op2 TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
